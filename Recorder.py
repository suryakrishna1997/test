...
class ADBInterface:
    ...
    def list_devices(self):
        ...
        if rc != 0:
            return []
        devices = []
        # output starts with "List of devices attached"
...
class ADBTestReplayApp:
    ...
    def _build_ui(self):
        ...
        left = ttk.LabelFrame(
            mid,
            text="Fallback: click canvas to add taps manually",
            padding=8,
        )
    ...
    def _initial_checks(self):
        if not self.adb.is_installed():
            ...
            messagebox.showerror(
                ...
                "Please install Android Platform Tools and restart.",
            )
            return
        self.log("ADB found.")
    ...
    def _refresh_device_info(self):
        ...
        if not devices:
            ...
            self.log("No ADB devices connected.", "WARN")
        ...
        if len(devices) == 1:
            ...
            self.adb.device = devices[0]
            self.log(
                f"Multiple devices found; using {devices[0]}.", "WARN"
            )
    ...
    def _record_worker(self):
        ...
        self.log("Starting getevent stream. Tap the device screen now.")
        try:
        ...
        try:
            for line in proc.stdout:
                ...
                if ev_type == EV_ABS:
                    ...
                    if pending_tap and cur_x is not None and cur_y is not None:
                        sx, sy = self._scale_raw_to_screen(cur_x, cur_y)
                        now = time.monotonic()
                        if last_tap_time is None:
                            delay_ms = 0
                        else:
                            delay_ms = int((now - last_tap_time) * 1000)
                        self.recorded_taps.append((sx, sy, delay_ms))
                        last_tap_time = now
                        self.log(
                            f"Tap #{len(self.recorded_taps)}: "
                            f"({sx},{sy}) delay={delay_ms}ms"
                        )
                    pending_tap = False
        except Exception as e:
            ...
            self.log(f"Recording stopped. Captured {count} tap(s).")
            self.set_summary(f"Recorded {count} tap(s).")
    ...
    def _replay_worker(self, iterations, inter_cycle_delay):
        ...
        for i in range(1, iterations + 1):
            ...
            for idx, (x, y, delay_ms) in enumerate(
                self.recorded_taps, start=1
            ):
                if self.stop_replay_flag.is_set():
                    break
                self.log(f"Tap {idx}/{n_taps}: ({x},{y}) +{delay_ms}ms")
                rc, out, err = self.adb.tap(x, y)
                if rc != 0:
                    self.log(f"Tap failed: {err}", "ERROR")
                    if not self.adb.device_online():
                        self.log(
    ...
    def on_canvas_click(self, event):
        ...
        sx = int(event.x * sw / cw)
        sy = int(event.y * sh / ch)

        # First tap has no preceding tap; later taps get the default delay.
        ...
        self.canvas.create_text(
            event.x,
            event.y - 12,
            text=str(len(self.recorded_taps)),
            fill="darkred",
            font=("TkDefaultFont", 8, "bold"),
        )
        self.log(
            f"Manual tap #{len(self.recorded_taps)} added: "
            f"({sx},{sy}) delay={delay_ms}ms"
        )
        self.set_summary(f"Recorded {len(self.recorded_taps)} tap(s).")
    ...
    def on_load_recording(self):
        ...
        try:
            with open(path) as f:
                data = json.load(f)
            self.recorded_taps = [
                (int(t[0]), int(t[1]), int(t[2])) for t in data.get("taps", [])
            ]
            if data.get("screen_size"):
                self.screen_size = tuple(data["screen_size"])
            self.log(f"Loaded {len(self.recorded_taps)} tap(s) from {path}")
