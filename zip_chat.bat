...
:: ===== ZIP Creation (Exclude Video, inside same folder) =====
echo.
echo Creating ZIP inside folder (excluding Video)...

powershell -Command "Compress-Archive -Path '%D%\Bugreport','%D%\Logcat','%D%\HS_Logs','%D%\Maker_Logs','%D%\MCU' -DestinationPath '%D%\%D%.zip' -Force"
